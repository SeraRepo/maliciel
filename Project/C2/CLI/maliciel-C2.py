...
listening_post_addr = "https://127.0.0.1:5000"
...
def api_get_request(endpoint):
    response_raw = requests.get(
        listening_post_addr + endpoint, verify=False).text
...
def api_post_request(endpoint, payload):
    response_raw = requests.post(
        listening_post_addr + endpoint, json=payload, verify=False).text
    response_json = json.loads(response_raw)
    return response_json
...
def list_tasks():
    ...
    api_endpoint = "/tasks"
    print("\nHere's the tasks:\n")
    pprint(api_get_request(api_endpoint))
    print()


@click.command(name="list-results")
def list_results():
    """Lists the results that are being served to the implant."""
...
def list_history():
    """Lists the history that are being served to the implant."""
...
@click.option('--options', help='Key-value options for tasks.')
def add_tasks(tasktype, options):
    """Submit tasks to the listening post."""
    api_endpoint = "/tasks"
    print("\nHere are the tasks that were added:\n")

    # Perfom options parsing if user provided them to the task
    if options != None:
        task_options_dict = {}
        ...
        # If more than one otpion was provided, format and append them into a single string
        if len(task_options_dict) > 1:
            ...
            pprint(api_post_request(api_endpoint, request_payload))

        # Otherwise, print the key/value for the single option provided
        else:
            request_payload_string = f'[{{"task_type":"{tasktype}","{key}":"{value}"}}]'
            request_payload = json.loads(request_payload_string)
            pprint(api_post_request(api_endpoint, request_payload))

    # Otherwise, submit a payload with the task specified
    else:
